...
// Mobile Full-Screen Homepage
document.addEventListener('DOMContentLoaded', function() {
    // Only run on mobile and only on homepage
    if (window.innerWidth <= 767 && !document.body.classList.contains('section-active') && 
...
function initMobileHomepage() {
    const sections = document.querySelectorAll('.section, .portrait-container');
    const paginationDots = document.querySelectorAll('.pagination-dot');
    const upArrow = document.querySelector('.mobile-nav-arrow.up');
    const downArrow = document.querySelector('.mobile-nav-arrow.down');
    let currentIndex = 0;
    let touchStartY = 0;
    let touchEndY = 0;
    ...
    sections.forEach((section, index) => {
        // Add section content container if not present
        if (!section.querySelector('.section-content')) {
            const content = document.createElement('div');
            content.className = 'section-content';
            
            // Move title and buttons into container
            const title = section.querySelector('h2, h3, .section-title');
            const button = section.querySelector('button, .expand-btn, .about-btn');
            
            if (title) content.appendChild(title.cloneNode(true));
            if (button) content.appendChild(button.cloneNode(true));
            
            section.appendChild(content);
        }
        
        // Find mobile section title or create from regular title
        const sectionContent = section.querySelector('.section-content');
        let mobileTitle = section.querySelector('.mobile-section-title');
        
        if (!mobileTitle && sectionContent) {
            const regularTitle = section.querySelector('.section-title');
            if (regularTitle) {
                mobileTitle = document.createElement('h2');
                mobileTitle.className = 'mobile-section-title';
                mobileTitle.textContent = regularTitle.textContent;
                mobileTitle.style.display = 'none';
                sectionContent.appendChild(mobileTitle);
            }
        }
    ...
    function updateNavigation() {
        ...
        upArrow.style.display = currentIndex > 0 ? 'flex' : 'none';
        downArrow.style.display = currentIndex < sections.length - 1 ? 'flex' : 'none';
    }
    ...
    // Handle button clicks in sections
    document.querySelectorAll('.expand-btn, .about-btn').forEach(button => {
    ...
    // Ensure the active section is visible on resize
    window.addEventListener('resize', () => {
        if (window.innerWidth > 767) {
            // Reset to desktop view
            sections.forEach(section => section.classList.remove('active'));
        } else {
            // Make sure current section is visible
